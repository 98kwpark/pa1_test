...
TableFunction PostgresTableEntry::GetScanFunction(ClientContext &context, unique_ptr<FunctionData> &bind_data) {
	...
	Value direct_read;
	if (context.TryGetCurrentSetting("pg_direct_read", direct_read) && BooleanValue::Get(direct_read)) {
		// direct read: resolve file path + physical layout, lock the table, count the blocks from the file
		PostgresDirectBind(context, transaction, *result);
		PostgresScanFunction::PrepareBind(pg_catalog.GetPostgresVersion(), context, *result,
		                                  static_cast<int64_t>(result->direct->nblocks));
		bind_data = std::move(result);
		return PostgresDirectScanFunction();
	}
	PostgresScanFunction::PrepareBind(pg_catalog.GetPostgresVersion(), context, *result,

...
struct PostgresLocalState : public LocalTableFunctionState {
	...
	//! block range of the current task (direct read; the COPY readers encode it in `sql`)
	idx_t task_min = 0;
	idx_t task_max = 0;
...
void PostgresScanFunction::PrepareBind(PostgresVersion version, ClientContext &context, PostgresBindData &bind_data,
	...
	if (!use_ctid_scan && !bind_data.use_direct_read) {
		approx_num_pages = 0;
	}
...
static void PostgresInitInternal(ClientContext &context, const PostgresBindData *bind_data_p,
	...
	if (bind_data->use_direct_read) {
		// no COPY statement: the reader gets the block range [task_min, task_max) of the heap file
		lstate.exec = false;
		lstate.done = false;
		lstate.task_min = task_min;
		lstate.task_max = task_max;
		lstate.sql.clear();
		return;
	}
...
static unique_ptr<GlobalTableFunctionState> PostgresInitGlobalState(ClientContext &context,
	...
	if (pg_catalog) {
		...
		if (bind_data.use_direct_read) {
			// lock + relfilenode/size check in the transaction that actually runs the scan
			PostgresDirectCheckAtScan(context, transaction, bind_data);
		}
		auto &con =
	...
	if (bind_data.requires_materialization) {
		...
		// (direct read does not query the server while scanning, so it needs no snapshot)
		if (!bind_data.use_direct_read) {
...
static bool PostgresParallelStateNext(ClientContext &context, const FunctionData *bind_data_p,
	...
	if (gstate.page_idx < bind_data->pages_approx) {
		...
		if (page_max >= bind_data->pages_approx || page_max > POSTGRES_TID_MAX) {
			...
			// (direct read counts the blocks from the file itself, so its last task ends there)
			page_max = bind_data->use_direct_read ? bind_data->pages_approx : POSTGRES_TID_MAX;
		}
...
static unique_ptr<LocalTableFunctionState> GetLocalState(ClientContext &context, TableFunctionInitInput &input,
	...
	if (!bind_data.use_direct_read && !gstate.TryOpenNewConnection(context, *local_state, bind_data)) {
...
void PostgresLocalState::ScanChunk(ClientContext &context, const PostgresBindData &bind_data,
	...
	if (!reader) {
		if (bind_data.use_direct_read) {
			reader = make_uniq<PostgresDirectReader>(connection, column_ids, bind_data);
	...
	while (true) {
		...
		if (!exec) {
			reader->SetTaskRange(task_min, task_max);
...
PostgresDirectScanFunction::PostgresDirectScanFunction()
    : TableFunction("postgres_direct_scan", {LogicalType::VARCHAR, LogicalType::VARCHAR, LogicalType::VARCHAR},
